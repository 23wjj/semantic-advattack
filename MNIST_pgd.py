...
for rate in range(1):
    ...
    def PGD(img, label, epsilon,alpha, mlp_encoder, mlp_mnist,device,iterations = 10):
        ...
        for t in range(iterations):
            ...
            img = img.detach()
            adv_img = img + alpha * sign_data_grad
            eta = torch.clamp(adv_img-img, min=-epsilon, max=epsilon)
            img = torch.clamp(img + eta, min=0, max=1).detach()
    ...
    def attack(mlp_encoder,mlp_mnist,device,dataloader,epsilon,alpha):
        correct=0
        orig_examples=[]
        adv_examples=[]
        init_preds=[]
        ...
        for data, target in dataloader:
            cnt+=1
            print("[Case] -",cnt)
            t,res,init_pred,final_pred,noise_img,out=PGD(data,target,epsilon,alpha,mlp_encoder,mlp_mnist,device)
            attack_cnt+=t
            correct+=res
